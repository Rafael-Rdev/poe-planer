...
try {
  ...
  // Firebase not configured yet – offline mode
}
...
export default function App() {
  ...
  useEffect(() => {
    if (!user || !db) return;
    const docRef = doc(db, 'artifacts', APP_ID, 'users', user.uid, 'settings', 'progress');
  ...
  const toggleAct = (id) => {
    const newActs = acts.map(act => act.id === id ? { ...act, done: !act.done } : act);
    setActs(newActs);
    if (user && db) {
  ...
  return (
    <div className="min-h-screen bg-black text-slate-200 font-sans pb-32 overflow-x-hidden">
          ...
          {activeTab === 'leveling' && (
            ...
            >
              {acts.map((act) => (
                <div
                  key={act.id}
                  className={`bg-white/[0.03] border rounded-[2.5rem] p-6 transition-all active:scale-[0.98] ${
                    act.done ? 'border-green-500/20 opacity-50' : 'border-white/10'
                  }`}
                >
                  ...
                  {!act.done && (
                    <div className="mt-5 space-y-4 pt-4 border-t border-white/5">
                      {act.steps.map((step, idx) => (
                        <div key={idx} className="flex items-start space-x-3 text-[13px] text-slate-400">
            ...
            >
              ...
              <div className="bg-white/[0.03] border border-white/10 rounded-[2.5rem] p-6">
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div className="bg-black/40 p-4 rounded-2xl border border-white/5 flex justify-between items-center font-bold">
                    <span className="text-red-400">Leben (Instant)</span>
                    <span className="text-[10px] uppercase text-slate-600">Nessa</span>
                  </div>
                  <div className="bg-black/40 p-4 rounded-2xl border border-orange-500/20 flex justify-between items-center font-bold">
                    <span className="text-orange-400 underline underline-offset-4">Rubin (Charge on Hit)</span>
                    <span className="text-[10px] uppercase text-orange-900 font-black">Pflicht</span>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="bg-red-500/10 border-2 border-red-500/20 rounded-[2.5rem] p-6 relative overflow-hidden">
                ...
                <p className="text-[13px] text-slate-200 font-bold leading-relaxed relative z-10">
                  ...
                  <span className="underline decoration-red-500">NIEMALS</span> mit Support-Gems!
                  Deine Mana-Reservierung steigt sonst über 100%.
                </p>
              </div>
              ...
              <div className="bg-white/[0.03] border border-white/10 rounded-[2.5rem] p-6">
                <h3 className="text-orange-400 font-black uppercase text-xs mb-4 tracking-widest flex items-center border-b border-white/5 pb-2">
                  <Package className="w-5 h-5 mr-2" /> Leveling Uniques
                </h3>
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex justify-between items-center group">
                      ...
                      <span className="bg-orange-500/10 text-orange-500 text-[10px] font-black px-2 py-1 rounded-lg shrink-0">
                        LVL {u.l}
                      </span>
                    </div>
                  ))}
        ...
        {[
          ...
          <button
            ...
            className={`flex flex-col items-center space-y-1 transition-all ${
              activeTab === tab.id ? 'text-orange-500 scale-110' : 'text-slate-600'
            }`}
          >
          ...
          >
            ...
            >
              ...
              <p className="text-slate-400 text-sm mb-8 leading-relaxed">
                Alle gesetzten Häkchen in den Akten werden unwiderruflich auf Null gesetzt.
              </p>
              <div className="flex space-x-3">
                <button
                  onClick={() => setShowResetConfirm(false)}
                  className="flex-1 bg-white/5 text-white font-bold py-4 rounded-2xl active:bg-white/10 transition-colors"
                >
                  Abbrechen
                </button>
                <button
                  onClick={resetProgress}
                  className="flex-1 bg-red-600 text-white font-bold py-4 rounded-2xl shadow-lg shadow-red-900/20 active:bg-red-700 transition-colors"
                >
                  Ja, Reset
                </button>
              </div>
